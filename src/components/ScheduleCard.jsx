...
function statusText(match) {
  const now = Date.now();
  const ts = typeof match.startTs === "number" ? match.startTs : null;
  if (match.status === "LIVE") return ts ? `LIVE • ${diffHM(now - ts)}` : "LIVE";
  if (match.status === "UPCOMING")
    return ts && ts > now ? `${tHM(new Date(ts))} • en ${diffHM(ts - now)}` : match.in || "UPCOMING";
  return "FINAL";
}
      ...
      <div className="sched__grid">
        ...
        <div className="time">
          ...
          {match.status === "COMPLETED" ? (
            <div className="time__clock">Final</div>
          ) : (
            <div className="time__clock">{ts ? tHM(ts) : match.in ?? "—"}</div>
          )}
          <div className="time__tz">{match.status === "COMPLETED" ? "" : ts ? tzAbbr(ts) : ""}</div>
        </div>
        ...
        <div className="cta">
          ...
          {match.id && (
            <Link href="#" className="cta__btn">{/* luego: `/matches/${match.id}` */}
              More info
            </Link>
          )}
        </div>
